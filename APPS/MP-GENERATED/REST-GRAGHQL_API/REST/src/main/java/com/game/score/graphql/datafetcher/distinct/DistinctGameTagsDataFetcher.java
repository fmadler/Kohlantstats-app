...
@Component
public class DistinctGameTagsDataFetcher implements DataFetcher<List<DistinctGameTagsOut>> {
    ...
    @Override
    public List<DistinctGameTagsOut> get(DataFetchingEnvironment dataFetchingEnvironment) {
        DistinctGameTagsIn distinctGameTagsIn = new DistinctGameTagsIn ();
        distinctGameTagsIn.setProgramWebPath (dataFetchingEnvironment.getArgument("programWebPath"));
        return distinctGameTagsDaoFace.execute (
        	distinctGameTagsIn
            ).getDistinctGameTagsOuts();
    }
