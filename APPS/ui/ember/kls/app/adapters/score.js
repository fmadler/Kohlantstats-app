...
export default class score extends JSONAPIAdapter {
  ...
  /* connector to ScoreTimeline info */
  scoreTimeline (params) {
    return fetchData(baseUrl+'/data/sdd/ScoreTimelineIn'+filterScoreTimeline(params));
  }
  /* pivot Participant */
  scoreTimelineGetParticipants (params) {
    return fetchData(baseUrl+'/data/sdd/ScoreTimelineIn/getParticipants'+filterScoreTimeline(params));
  }
  /* pivot Result */
  scoreTimelineGetResults (params) {
    return fetchData(baseUrl+'/data/sdd/ScoreTimelineIn/getResults'+filterScoreTimeline(params));
  }
...
function filterMasterData (filter) {
  if (filter) {
    return appendQuery("",{
        "table":filter.table,	//Allowed values gs_team_departure_type,gs_team_entrance_type,gs_team_reward_type,gs_game_type,gs_game_stake_type,gs_realisation_type,gs_vote_type
...
/* filter for ScoreTimeline entity */
function filterScoreTimeline (filter) {
  if (filter) {
    return appendQuery("",{
        "programWebPath":filter.programWebPath,
        "playerWebPaths":filter.playerWebPaths,
...
/* filter for CreateTimeline entity */
function filterCreateTimeline (filter) {
  if (filter) {
    return appendQuery("",{
        "programWebPath":filter.programWebPath,
